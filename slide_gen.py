...
| `GSI2_PK` | String | `USER#{owner_id}` （オーナーのショップ一覧取得用） |""")
    ...
    # Slide 9: Data Structure - Product 1
    slides.append("""---
...
| `price` | Number | 価格 （0以上の正の数値） |""")
    ...
    # Slide 10: Data Structure - Product 2
    slides.append("""---
...
| `GSI2_PK` | String | `SHOP#{shopId}` （担当ショップ一覧取得用） |""")
    ...
    # Slide 13: Data Structure - Order 1
    slides.append("""---
...
| `preferredDate` | String | 配達希望日付 （YYYY-MM-DD形式等） |""")
    ...
    # Slide 14: Data Structure - Order 2
    slides.append("""---
...
- (例外2) `BANNED`: 管理者による不正利用・強制停止
  ...
  - QR機能が完全に停止され、ユーザー・ショップ双方の操作を遮断""")
    ...
    # Slide 18: Function Flow - Authentication
    slides.append("""---
...
- 新規登録・初期認証 (`/register`, `/verify`)
  ...
  - 検証通過により正式なアクセス権限(Cognito Account)の確立""")
    ...
    # Slide 19: Function Flow - Recipient 1
    slides.append("""---
...
- デバイス操作: 受取手続き (`ACTIVE` → `USED`)
  ...
  - 氏名、配送先住所、電話番号、配達希望日時の送信処理""")
    ...
    # Slide 20: Function Flow - Recipient 2
    slides.append("""---
...
- 問い合わせ（チャット）機能
  ...
  - Eメールによる新着通知購読の対応設定""")
    ...
    # Slide 21: Function Flow - Shop Admin 1
    slides.append("""---
...
- 商品 (Product) 管理機能
  ...
  - 取扱い停止 (`STOPPED`) および安全ステータス確認後の商品削除対応""")
    ...
    # Slide 22: Function Flow - Shop Admin 2
    slides.append("""---
...
- 配送・受注管理機能
  ...
  - 配送業者名・追跡番号登録のよる `SHIPPED` への状態移行（発送処理）""")
    ...
    # Slide 23: Function Flow - System Admin
    slides.append("""---
...
- システム監視・データ統制機能
  - ステータス別集計、UUID個別照会、不正QRの `BANNED` 化および完全削除""")
    ...
    # Slide 24: Backend Logic - DynamoDB CDK
    slides.append("""---
...
```""")
    ...
    # Slide 25: Backend Logic - DynamoDB Put/Get
    slides.append("""---
...
- アカウント保護
  - Cognitoの強固なパスワードポリシー制約（8文字・英大小・数字必須）""")
    ...
    # Slide 27: Security - Tenant Isolation
    slides.append("""---
...
```""")
    ...
    # Slide 29: Security - Rate Limiting
    slides.append("""---
...
- 認証失敗の記録化と一時ロック機能
  ...
  - アクセス初回にロック状態を検証し、ロック中なら即座に `403 Forbidden` を返却""")
    ...
    # Slide 30: Security - Transactions
    slides.append("""---
...
```""")
    ...
    # Slide 32: External Integrations
    slides.append("""---
...
- 依存関係のインストールと環境起ち上げ
  ...
  - `npm run dev` によるローカル画面の確認 (`localhost:3000`)""")
    ...
    # Slide 35: Dev Workflow Frontend
    slides.append("""---
...
- 異なるオーナー間でのインポート（承認システム）
  ...
  - 解決策案: トークン配布による認証通過、またはシステム画面上の申請(`IMPORT_REQUEST`)からの明示的承諾フローの確立""")
    ...
    # Write the file
    with open('c:\\git\\meishigawarini\\slide2.md', 'w', encoding='utf-8') as f:
        f.write("\n".join(slides))
        f.write("\n")
    ...
    for i, slide in enumerate(slides):
        ...
        if len(lines) > 12:
            print(f"Slide {i+1} has {len(lines)} lines! Max is 12.")
            print("\n".join(lines))
            success = False
            
    if success:
        print(f"Successfully generated {len(slides)} slides. All limits met.")
