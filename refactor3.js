...
`;
    ...
    const endOfFileIndex = content.lastIndexOf('</Card>');
    ...
    if (
        ...
        // Status Guide block from Order History to the last Card
        const historyToStatusGuide = content.substring(orderHistoryIndex, endOfFileIndex + '</Card>'.length);
        ...
        // Find the remaining part of the file after the last Card (which is just closing divs)
        prefix += `                    </div>\n                )}\n` + content.substring(endOfFileIndex + '</Card>'.length);

        fs.writeFileSync(filePath, prefix, 'utf8');
        console.log("Refactored successfully");
    } else {
        console.error("Could not find sections indices.", { linkQrIndex, incomingOrdersIndex, existingProductsIndex, orderHistoryIndex, statusGuideIndex, endOfFileIndex });
        process.exit(1);
    }
} catch (e) {
