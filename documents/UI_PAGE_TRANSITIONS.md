...
* **可能な操作一覧**:
  ...
  * **カードデザインの管理 (Card Design Editor)**:
    * **デザインの新規作成・編集**: カードの両面背景画像、QRコード、PIN、UUIDなどの配置をミリメートル単位で詳細に設定可能です。
    * **リアルタイムプレビュー**: 編集中の設定を即座にシミュレートし、実際のカードサイズに基づいたプレビューを表示します。
    * **背景画像のアップロードと自動軽量化**: 背景画像をアップロードすると、クライアントサイドで400pxのWebPサムネイルが自動生成され、一覧表示の高速化に寄与します。
    * **デザインの削除**: デザインを削除すると、関連するS3上の画像フォルダ（背景・サムネイル）も再帰的に全て削除されます。
