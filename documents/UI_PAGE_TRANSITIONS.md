...
アプリケーションは大きく「**一般・受取人向けページ**」「**ショップ管理者向けページ**」「**システム管理者向けページ**」の3つに分かれます。
...
graph TD
    ...
    Verify -->|登録完了| Login
    Login -->|ログイン成功| ShopList

    ShopList -->|ショップ作成/選択| ShopDetail
    Login -->|管理者権限あり＆リンク直打ち| Admin
```
...
### a. ショップ一覧 (`/[locale]/shop`)
* **概要**: ログイン済みユーザーが管理している（権限をもつ）ショップの一覧を表示するダッシュボードの入り口。
* **可能な操作**:
  ...
  * **新規ショップの作成**: 「新規ショップ作成」ボタンを押下するとダイアログ・モーダルが開き、ショップ名を入力して作成処理を実行できます。バックエンドAPIがコールされてDBに新規店舗レコードが作成され、完了後に一覧表示が自動更新されて画面に反映されます。
...
### a. システム管理ダッシュボード (`/[locale]/admin`)
* **概要**: 全体システムのQRコード一括生成、状態の個別監視、バン(Ban)処理を行う、オーナー（Administratorsグループ）のみがアクセス可能なページ。
* **可能な操作一覧**:
