...
*   **仕組み**:
    ...
    6.  **外部認証アカウントに関する制限**: 
        *   Google/Apple等の外部Idp経由でサインアップしたユーザーは、CognitoのネイティブMFA（TOTP）を設定することができません。
        *   そのため、管理権限を付与してもLambda AuthorizerのMFAチェックをパスすることができず、実質的に管理機能を利用できません。
        *   **管理者を設定する際は、必ずCognitoの「メールアドレスとパスワード」で新規作成したユーザーを使用してください。**
