...
sequenceDiagram
    ...
    SA->>SA: 1.1 アカウント作成 (/login または /login)
...
sequenceDiagram
  ...
  SO->>SO: 2.1 ユーザーアカウント作成 (/login)
  SO->>SYS: 2.2 ショップ開設申請 (/user の「申請・チャット」→「ショップ開設申請」)
...
| :--- | :--- | :--- | :--- | :--- |
| 2.1 | ユーザーアカウント作成 | ショップオーナー | `/login` | ショップを開設する一般ユーザーとしてサインアップします。 |
| 2.2 | **ショップ開設申請** | ショップオーナー | `/user` (マイページ) > **「申請・チャット」** > **「ショップ開設申請」** | ダイアログが開き、「ショップ名・申請者名・備考」を入力して送信します。送信すると `SHOP_OPENING` タイプのチャットが作成され、管理者に通知されます。 |
| 2.2a | **運営とチャット（一般問い合わせ）** | ショップオーナー | `/user` (マイページ) > **「申請・チャット」** > **「運営とチャット」** | ダイアログで初期メッセージを入力して送信します。送信すると起票者が `USER#...` の場合は `USER_SUPPORT`、`SHOP#...` の場合は `SHOP_SUPPORT` タイプのチャットが作成され、運営とのやり取りを開始できます。 |
...
| **一般問い合わせチャット種別** | 起票者が `USER#...` の場合 `chat_type: "USER_SUPPORT"`、`SHOP#...` の場合 `chat_type: "SHOP_SUPPORT"` |
