/**
 * @file shop_products_import.ts
 * @role ショップ用：商品情報のインポート（ショップ間コピー）ハンドラー
 * @responsibility
 *  - 別のショップ（ソースショップ）に登録されている商品情報を、自身のショップへコピーしてインポートします。
 *  - 【アセットの完全継承】商品名、説明、価格だけでなく、商品画像の S3 ファイルも自ショップ用ディレクトリへ物理コピー（CopyObject）し、URL を整合性を保って更新します。
 *  - 【権限の交差検証】インポート元とインポート先の両方のショップに対して、実行ユーザーが適切な管理権限を持っているかを厳格にチェックします。
 * @context
 *  - 複数店舗を展開するオーナーが、既存店舗のメニューを新店舗へ一括反映させる際の利便性向上を目的としています。
 */
